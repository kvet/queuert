...
type MergedDefs = JobTypeRegistryDefinitions<typeof orderJobTypes> &
  JobTypeRegistryDefinitions<typeof notificationJobTypes>;

export const createOrderProcessors = (client: { startJobChain: (...args: any[]) => any }) =>
  ({
    "orders.create-order": {
      ...
      attemptHandler: async ({ job, complete }) => {
        ...
        return complete(async ({ continueWith, sql, transactionHooks }) => {
          ...
          return continueWith({
            ...
            blockers: [notifyChain as Parameters<typeof continueWith>[0]["blockers"][0]],
          });
        });
      },
    },
  ...
  }) satisfies InProcessWorkerProcessors<typeof stateAdapter, MergedDefs>;

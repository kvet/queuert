...
/**
 * A schema migration applied once per database. Identified by `name` (used as
 * the dedup key in the migrations table) and a list of SQL `statements` run
 * in order.
 *
 ...
 * - `false` — statements run outside any transaction. Use only when the DB
 *   forbids transactional execution (e.g., Postgres `CREATE INDEX CONCURRENTLY`).
...
export const executeMigrations = async <TTxContext>({
  ...
  runInTransaction: <T>(cb: (txCtx: TTxContext) => Promise<T>) => Promise<T>;

...
/** The adapter's id data type: `string` for every `idType` SQLite accepts. */
...
export const createLegacyUpgrade = <TTxContext extends BaseTxContext>(
  ...
  // so the import cannot drift away from how the rest of the adapter marshals an id.
...
SELECT 1 FROM {{table_prefix}}migration WHERE name = '${upgradeFloor}'
  ...
  const renameAsideStatements = [
    // SQLite rewrites the REFERENCES clauses of other tables when a table is
    // renamed, so job_blocker_old ends up pointing at job_old.
    ...
    // Index names are global and SQLite has no ALTER INDEX RENAME, so the names
    // the new install wants back are dropped and the one the import needs is recreated.
...
ON {{table_prefix}}job_old (chain_id, chain_index)`,
    ...
    // current schema instead of reporting migrations it has never heard of.
...
WHERE chain_index = 0 AND id > ? ORDER BY id LIMIT ?`,
  ...
  // `completed_at`, the tail is the successor-less row the self-join already
  // identifies, and `chain_status` follows from the same value. Batching also keeps
  // `job_blocker.job_id`'s foreign key satisfiable. `status` carries the same four
  // values in both models. `attempt_at` did not exist in v0.15 and a running row may
  // carry no lease at all; workers are stopped during an upgrade, so every running
  // attempt is orphaned and starts over from now. The chain columns — `chain_status`,
  // `chain_completed_at`, `deduplication_key`, `chain_trace_context` — are written on
  // head rows only.

...
export type MysqlStateAdapter = StateAdapter<MysqlPoolContext, string>;

export const extendWithStateMysql = <
  T extends {
...
> => {
  return api.extend<{
    ...
    stateAdapter: MysqlStateAdapter;
    flakyStateAdapter: MysqlStateAdapter;
  }>({
    statePool: [
      async ({ mysqlConnectionString }, use) => {
  ...
  }) as ReturnType<typeof extendWithStateMysql<T>>;
};

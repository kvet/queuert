...
describe("JobTypeEntryDefinitions", () => {
  it("includes only job types with entry: true", () => {
    ...
    type EntryDefs = JobTypeEntryDefinitions<Defs>;
...
describe("BlockerChains", () => {
  ...
  it("resolves cross-slice blocker output via merged definitions (tuple)", () => {
    type NotifDefs = {
      "notif.send": {
        entry: true;
        input: { userId: string };
    ...
    type MergedDefs = OrderDefs | NotifDefs;

    type Blockers = CompletedBlockerChains<string, MergedDefs, "orders.confirm">;
    expectTypeOf<Blockers[0]["output"]>().toEqualTypeOf<{ sentAt: string }>();
  });

  it("resolves cross-slice blocker output via merged definitions (array)", () => {
    type NotifDefs = {
      "notif.send": {
        entry: true;
        input: { userId: string };
    ...
    type Chains = BlockerChains<string, MergedDefs, "orders.confirm">;
    type Blockers = CompletedBlockerChains<string, MergedDefs, "orders.confirm">;
...
describe("JobTypeBlockedNames", () => {
  it("resolves job types blocked by a given chain type", () => {
    ...
    expectTypeOf<JobTypeBlockedNames<Defs, "dep">>().toEqualTypeOf<"main">();
  });
  ...
  it("returns never when no job types are blocked by the chain", () => {
    ...
    expectTypeOf<JobTypeBlockedNames<Defs, "standalone">>().toEqualTypeOf<never>();
  });
  ...
  it("resolves multiple job types blocked by the same chain", () => {
    ...
    expectTypeOf<JobTypeBlockedNames<Defs, "auth">>().toEqualTypeOf<"taskA" | "taskB">();
  });
  ...
  it("handles multiple blockers on a single job type", () => {
    ...
    expectTypeOf<JobTypeBlockedNames<Defs, "depA">>().toEqualTypeOf<"main">();
    expectTypeOf<JobTypeBlockedNames<Defs, "depB">>().toEqualTypeOf<"main">();
  });
});
...
describe("rest/variadic blocker slots", () => {
  ...
  describe("ResolvedJob narrowing on continueWith presence", () => {
    type Defs = {
      // Has continueWith → can be terminal OR continued
      step: {
        entry: true;
        input: { n: number };
        output: { final: boolean };
        continueWith: { typeName: "step" };
      };
      // No continueWith → terminal-only
      terminal: {
        entry: true;
        input: { id: string };
        output: { processed: true };
      };
      // No output declared → continued-only (output: never eliminates terminal variant)
      relay: {
        entry: true;
        input: { url: string };
        continueWith: { typeName: "terminal" };
      };
    };
    ...
    it("a job type without continueWith narrows to terminal-only", () => {
      ...
      type ContinuedSibling = Extract<Completed, { continuedToId: string }>;

      expectTypeOf<Completed["output"]>().toEqualTypeOf<{ processed: true }>();
      expectTypeOf<Completed["continuedToId"]>().toEqualTypeOf<null>();
    ...
    it("a job type with continueWith keeps both completed variants", () => {
      ...
      type Completed = Extract<J, { status: "completed" }>;
      type Terminal = Extract<Completed, { continuedToId: null }>;
      type Continued = Exclude<Completed, { continuedToId: null }>;

      expectTypeOf<Terminal["output"]>().toEqualTypeOf<{ final: boolean }>();
      expectTypeOf<Terminal["continuedToId"]>().toEqualTypeOf<null>();
      expectTypeOf<Continued["continuedToId"]>().toEqualTypeOf<string>();

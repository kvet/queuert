...
export const clientQueriesTestSuite = ({ it: baseIt }: { it: TestAPI<TestSuiteContext> }): void => {
  ...
  describe("getChain", () => {
    ...
    it("getChain returns chain by id", async ({ client, withTransaction, expect }) => {
    ...
    it("getChain narrows return type by typeName", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getChain returns without typeName", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getChain throws on typeName mismatch", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
  ...
  describe("getChains", () => {
    it("getChains returns empty array for empty ids", async ({ client, expect }) => {
      const chains = await client.getChains({ ids: [] });

      expect(chains).toEqual([]);
    });

    it("getChains returns undefined for nonexistent ids", async ({ client, expect }) => {
    ...
    it("getChains returns chains in input order", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
          typeName: "order",
      ...
      const notification = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
          typeName: "notification",
    ...
    it("getChains narrows return type by typeName", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getChains throws on typeName mismatch", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getChains skips typeName check for undefined entries", async ({ client, expect }) => {
      const chains = await client.getChains({
        typeName: "order",
        ids: ["00000000-0000-0000-0000-000000000000"],
      });
  ...
  describe("getJob", () => {
    it("getJob returns undefined for nonexistent job", async ({ client, expect }) => {
    ...
    it("getJob returns job by id", async ({ client, withTransaction, expect }) => {
    ...
    it("getJob returns without typeName", async ({ client, withTransaction, expect }) => {
      const chain = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getJob throws on typeName mismatch", async ({ client, withTransaction, expect }) => {
      const chain = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
  ...
  describe("getJobs", () => {
    it("getJobs returns empty array for empty ids", async ({ client, expect }) => {
      const jobs = await client.getJobs({ ids: [] });

      expect(jobs).toEqual([]);
    });

    it("getJobs returns undefined for nonexistent ids", async ({ client, expect }) => {
    ...
    it("getJobs returns jobs in input order", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getJobs narrows return type by typeName", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getJobs throws on typeName mismatch", async ({ client, withTransaction, expect }) => {
      const created = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("getJobs skips typeName check for undefined entries", async ({ client, expect }) => {
      const jobs = await client.getJobs({
        typeName: "order",
        ids: ["00000000-0000-0000-0000-000000000000"],
      });
  ...
  describe("listChainTypeNames", () => {
    it("listChainTypeNames returns empty array when no chains exist", async ({
      client,
      expect,
    }) => {
    ...
    it("listChainTypeNames returns sorted distinct chain type names", async ({
      client,
      withTransaction,
      expect,
    }) => {
      ...
      expect(names).toEqual(["notification", "order"]);
    });

    it("listChainTypeNames does not include continuation-only type names", async ({
      client,
      withTransaction,
      expect,
    }) => {
      ...
      await withTransaction(async (txCtx, transactionHooks) =>
        client.completeChain({
          ...
          handler: async ({ job, completeJob }) => {
            if (job.typeName === "order") {
              return completeJob(job, async ({ finish }) =>
                finish({
                  continueWith: { typeName: "order_fulfill", input: { orderId: "o1" } },
                }),
              );
            }
          },
  ...
  describe("listJobTypeNames", () => {
    it("listJobTypeNames returns empty array when no jobs exist", async ({ client, expect }) => {
      const names = await client.listJobTypeNames();

      expect(names).toEqual([]);
    });

    it("listJobTypeNames returns sorted distinct job type names", async ({
      client,
      withTransaction,
      expect,
    }) => {
    ...
    it("listJobTypeNames includes continuation job type names", async ({
      client,
      withTransaction,
      expect,
    }) => {
      ...
      expect(names).toEqual(["order", "order_fulfill"]);
    });
  });

  describe("countByChainTypeNames", () => {
    it("returns empty array for empty input", async ({ client, expect }) => {
      const counts = await client.countByChainTypeNames({ typeNames: [] });
      expect(counts).toEqual([]);
    });

    it("counts running chains", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({ ...txCtx, transactionHooks, typeName: "order", input: { amount: 1 } }),
      );
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
      ...
      const counts = await client.countByChainTypeNames({
        typeNames: ["order", "notification", "report"],
      });
    ...
    it("counts completed chains", async ({
      stateAdapter,
      notifyAdapter,
      observabilityAdapter,
      log,
      ...
      const jobTypes = defineJobTypes<{
        task_a: { entry: true; input: null; output: null };
        task_b: { entry: true; input: null; output: null };
      }>();
      ...
      const counts = await client.countByChainTypeNames({ typeNames: ["task_a", "task_b"] });

      expect(counts[0].completed.count).toBeGreaterThanOrEqual(1);
      expect(counts[0].running.count + counts[0].completed.count).toBe(2);
      expect(counts[1].running.count + counts[1].completed.count).toBeGreaterThanOrEqual(1);
    });
  });

  describe("countByJobTypeNames", () => {
    it("returns empty array for empty input", async ({ client, expect }) => {
      const counts = await client.countByJobTypeNames({ typeNames: [] });
      expect(counts).toEqual([]);
    });

    it("counts pending jobs", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({ ...txCtx, transactionHooks, typeName: "order", input: { amount: 1 } }),
      );
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({ ...txCtx, transactionHooks, typeName: "order", input: { amount: 2 } }),
      );
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
      ...
      const counts = await client.countByJobTypeNames({
        typeNames: ["order", "notification", "report"],
      });
    ...
    it("counts running and completed jobs", async ({
      stateAdapter,
      notifyAdapter,
      observabilityAdapter,
      log,
      ...
      const toComplete = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({ ...txCtx, transactionHooks, typeName: "task_a", input: null }),
      );
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({ ...txCtx, transactionHooks, typeName: "task_b", input: null }),
      ...
      await withWorkers([await worker.start()], async () => {
        await client.awaitChain(toComplete, completionOptions);
      });

      const counts = await client.countByJobTypeNames({ typeNames: ["task_a", "task_b"] });

      expect(counts[0].completed.count).toBeGreaterThanOrEqual(1);
      expect(counts[0].pending.count + counts[0].running.count + counts[0].completed.count).toBe(2);
      expect(
        counts[1].pending.count + counts[1].running.count + counts[1].completed.count,
      ).toBeGreaterThanOrEqual(1);
    });
  });

  describe("listChains", () => {
    it("listChains returns empty page when no chains exist", async ({ client, expect }) => {
      const page = await client.listChains({ typeName: "order" });

      expect(page.items).toEqual([]);
      expect(page.nextCursor).toBeNull();
    });

    it("listChains returns chains of the given type", async ({
      client,
      withTransaction,
      expect,
    }) => {
    ...
    it("listChains filters root-only (excludes blocker chains)", async ({
      ...
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...
          typeName: "report",
          input: { type: "summary" },
          blockers: [order] as never,
        }),
      );

      const allChains = await client.listChains({ typeName: "order" });
      const rootChains = await client.listChains({ typeName: "order", independent: true });

      expect(allChains.items).toHaveLength(1);
      expect(rootChains.items).toHaveLength(0);
    });

    it("listChains filters by status", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const running = await client.listChains({ typeName: "order", status: "running" });

      expect(running.items).toHaveLength(1);
    });

    it("listChains orders ascending", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const desc = await client.listChains({ typeName: "order" });
      const asc = await client.listChains({ typeName: "order", orderDirection: "asc" });
    ...
    it("listChains paginates with cursor", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const page1 = await client.listChains({ typeName: "order", limit: 2 });
    ...
    it("listChains filters by date range", async ({ client, withTransaction, expect }) => {
      const first = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("listChains sorts completed chains by completedAt when orderBy is completedAt", async ({
      ...
      await sleep(5);
      const chainB = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({ ...txCtx, transactionHooks, typeName: "task_a", input: null }),
      );
    ...
    it("listChains returns correct chain shape", async ({ client, withTransaction, expect }) => {
      const chain = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
  ...
  describe("listJobs", () => {
    it("listJobs returns empty page when no jobs exist", async ({ client, expect }) => {
      const page = await client.listJobs({ typeName: "order" });

      expect(page.items).toEqual([]);
      expect(page.nextCursor).toBeNull();
    });

    it("listJobs returns jobs of the given type", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("listJobs filters by status", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const pending = await client.listJobs({ typeName: "report", status: "pending" });
      const blocked = await client.listJobs({
        typeName: "report",
        status: "pending",
        blocked: true,
      });
      const runnable = await client.listJobs({
        typeName: "order",
        status: "pending",
        blocked: false,
      });
      ...
      expect(pending.items[0].status === "pending" && pending.items[0].blocked).toBe(true);
      ...
      expect(runnable.items).toHaveLength(1);
      expect(runnable.items[0].typeName).toBe("order");
    });

    it("listJobs orders ascending", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const desc = await client.listJobs({ typeName: "order" });
      const asc = await client.listJobs({ typeName: "order", orderDirection: "asc" });
    ...
    it("listJobs paginates with cursor", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const page1 = await client.listJobs({ typeName: "order", limit: 2 });
      expect(page1.items).toHaveLength(2);
      expect(page1.nextCursor).not.toBeNull();
    ...
    it("listJobs filters by date range", async ({ client, withTransaction, expect }) => {
      const first = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("listJobs returns correct job shape", async ({ client, withTransaction, expect }) => {
      await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
  ...
  describe("listChainJobs", () => {
    it("listChainJobs returns empty page for nonexistent chain", async ({ client, expect }) => {
    ...
    it("listChainJobs throws on typeName mismatch", async ({ client, withTransaction, expect }) => {
      const chain = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
  ...
  describe("getJobBlockers", () => {
    ...
    it("getJobBlockers returns blocker chains", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
      ...
      const report = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
          typeName: "report",
  ...
  describe("listBlockedJobs", () => {
    ...
    it("listBlockedJobs paginates", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
    ...
    it("listBlockedJobs orders ascending", async ({ client, withTransaction, expect }) => {
      const order = await withTransaction(async (txCtx, transactionHooks) =>
        client.createChain({
          ...txCtx,
          transactionHooks,
  ...
  describe("cross-method integration", () => {
    it("query methods return consistent data after chain completion", async ({
      ...
      const jobs = await client.listJobs({ typeName: "task" });
    ...
    it("query methods work with blockers across chains", async ({
      ...
      const { depChain, mainChain } = await withTransaction(async (txCtx, transactionHooks) => {
        const depChain = await client.createChain({
          ...txCtx,
          transactionHooks,
          typeName: "dep",
    ...
    it("default limit is applied when not specified", async ({
      ...
      const chains = await client.listChains({ typeName: "order" });
      const jobs = await client.listJobs({ typeName: "order" });

...
export const chainsTestSuite = ({ it }: { it: TestAPI<TestSuiteContext> }): void => {
  ...
  it("handles chained jobs", async ({
    ...
    expectTypeOf<
      Parameters<(typeof client)["createChain"]>[0]["typeName"]
  ...
  it("handles branched chains", async ({
    ...
    const evenChain = await withTransaction(async (txCtx, transactionHooks) =>
      client.createChain({
        ...txCtx,
        transactionHooks,
        typeName: "main",
    ...
    const oddChain = await withTransaction(async (txCtx, transactionHooks) =>
      client.createChain({
        ...txCtx,
        transactionHooks,
        typeName: "main",
  ...
  it("independent chains created during job processing do not inherit context", async ({
    ...
    const parentChain = await withTransaction(async (txCtx, transactionHooks) =>
      client.createChain({
        ...txCtx,
        transactionHooks,
        typeName: "parent",

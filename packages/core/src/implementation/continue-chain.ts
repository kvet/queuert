...
/**
 * Commits the `{ continueWith }` outcome: one adapter call inserts the successor
 * and completes `fromJob` pointing at it. The successor's `job_created` event is
 * buffered ahead of the predecessor's completion events.
 *
 * @param options.fromJob - Predecessor being continued. The worker passes a copy
 * carrying the live attempt span's trace contexts.
 */

...
import { EnqueueDependencyJobChains, queuertHelper, RetryConfig } from "./queuert-helper.js";
...
export { rescheduleJob, type RetryConfig } from "./queuert-helper.js";
export { type JobAttemptError } from "./state-adapter/state-adapter.js";
...
> = {
  setupQueueHandler: <
    ...
    TDependencies extends readonly ResolvedJobChain<TQueueDefinitions, keyof TQueueDefinitions>[],
  >(options: {
    name: TQueueName;
    enqueueDependencyJobChains?: EnqueueDependencyJobChains<
      ...
      TDependencies
    >;
    handler: JobHandler<TStateProvider, TQueueDefinitions, TQueueName, TDependencies>;
  ...
  return {
    createWorker: () => {
      const createWorkerInstance = (
        ...
        return {
          setupQueueHandler({ name: queueName, enqueueDependencyJobChains, handler, retryConfig }) {

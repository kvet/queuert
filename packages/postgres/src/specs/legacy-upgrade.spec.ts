...
describe("upgrade phases", () => {
  it(
    ...
    async ({ loaded: { provider } }) => {
      ...
      expect(await indexNames(provider)).toContain("queuert_job_ready_idx");
...
/** A v0.15.1 database in miniature: one continued chain, one blocked job, one running job. */
...
describe("custom id types", () => {
  // `pg` sends every parameter untyped, so an id parameter the import declares as
  // the wrong data type only fails against a provider that honours the declaration.
  it(

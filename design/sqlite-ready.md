...
- **Provider contract**: `SqliteStateProvider.withTransaction` gives exclusive atomic access. *How* a provider achieves that — connection-per-tx, single-writer pool, ORM-native transaction — is the provider author's choice. Document the contract; do not prescribe one strategy.
- **User-managed transaction contract**: when a user shares their own SQLite transaction with queuert (passing their own `txCtx` to mutating client methods like `deleteJobChains`, `triggerJob`, `completeJobChain`), they must start it with `BEGIN IMMEDIATE`. All mutating state-adapter operations are read-modify-write (e.g. `deleteJobChains` does check-then-delete: scan for external blockers, then delete blockers and chains). Under `BEGIN DEFERRED` in WAL mode, the upgrade can fail mid-transaction with `SQLITE_BUSY_SNAPSHOT` after wasted reads, instead of failing cleanly at `BEGIN`. Snapshot isolation prevents data corruption either way — this is an ergonomics/contract concern, not a safety one. The bundled `withTransaction` already does the right thing; the doc note is for users who manage their own transaction.
...
- Document that user-managed transactions sharing a `txCtx` with queuert mutating methods must use `BEGIN IMMEDIATE` (all mutating state-adapter ops are read-modify-write; deferred BEGIN risks `SQLITE_BUSY_SNAPSHOT` mid-transaction).

...
- **`BEGIN` (DEFERRED) instead of `BEGIN IMMEDIATE`** _(landed)_. The bundled providers, the example providers, the example `index.ts` user-managed-tx demos, and the conformance specs now all start transactions with plain `BEGIN`. Combined with the operation-level locking above, transactions promote to RESERVED only when actually needed (`createChains`, `startJobAttempt`, `lock: "exclusive"` reads, etc.), instead of locking the database the moment `withTransaction` is entered. Under the current single-connection + `acquireWrite()` rwlock, every transaction still serializes JS-side, so this is a no-op for today's tests; it becomes load-bearing once the rwlock is dropped for pooled WAL connections.
- **Write promotion via a sentinel table**. The no-op `UPDATE ... SET id = id` above targets the rows being read, which buys nothing (SQLite's write lock is database-wide) and forces `getChainsByDeduplication` to carry a duplicate of its resolve subquery. Replace it with a single cached write to a dedicated sentinel table, and decide whether promotion happens eagerly at `withTransaction` entry or lazily at the first write-intent statement. This gates dropping the rwlock: today's single-connection setup has no competing writer, so lazy promotion cannot fail — under pooled WAL connections it can fail with `SQLITE_BUSY_SNAPSHOT`, which `busy_timeout` does not cover. See [sqlite-write-promotion.md](./sqlite-write-promotion.md).
- **User-managed transaction contract** _(deferred)_. The original plan was to require user-managed transactions sharing a `txCtx` with queuert mutating methods to use `BEGIN IMMEDIATE`, so the upgrade-to-RESERVED can't fail mid-transaction with `SQLITE_BUSY_SNAPSHOT` after wasted reads. With operation-level `lock: "exclusive"` now landed, that recommendation becomes specific to the WAL+pool deployment story — under single-connection (today's examples and tests) there is no contention to fail on, so we don't need to prescribe it anywhere yet. Note that it can only ever be a recommendation: ORM-native `db.transaction()` (kysely, drizzle, prisma) issues plain `BEGIN` with no way to ask for `IMMEDIATE`, so the adapter cannot rely on it. Re-introduce the recommendation alongside the multi-worker WAL example.

...
`Job` no longer carries `chainTypeName`. A job's own `typeName` and `chainId` identify it; the chain's type belongs to the chain, so read it from `getChain`/`listChains` (or `chain.typeName` inside `completeChain`) instead of off each job

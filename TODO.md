...
- [EPIC] Align implementation with `design/job-model.md` — rename `has_blockers` → `has_open_blockers`, expand `JobStatus` from 4 values to 6 (`blocked` / `scheduled` / `ready` / `running` / `continued` / `completed`) as a discriminated union with `openBlockerChainIds` on the blocked variant, add `last_user_reschedule_attempt int NULL`, rename `ChainStatus` to `'open' | 'closed'`, add `UNIQUE (chain_id) WHERE continued_to_job_id IS NULL` partial, rebuild dashboard listing partials to match the new derivation. Migration is mostly column/index renames + one additive column. See `design/job-model.md` for the spec and `design/job-model.md#migration-from-current-state` for the delta list.

...
- [?,REF] `wrapHandler` cannot observe attempt failures — `runJobAttempt` catches handler/callback errors, reschedules, and returns normally, so a `catch` around `next()` never runs (only `finally` does). Decide whether middleware should see the error (e.g. pass it to a `onAttemptError` hook or rethrow after the reschedule commits) or whether the current behavior is intentional and only needs documenting.

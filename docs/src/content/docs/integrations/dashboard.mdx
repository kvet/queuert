---
title: Dashboard
description: Embeddable web UI for observing chains and jobs.
sidebar:
  order: 5
---
...
- **Job detail** -- Input/output data, error messages, attempt info, continuation links
...
<Aside type="caution" title="Performance on large tables">
...
![Chain list view showing chains with status badges and input previews](../../../assets/dashboard/chains-list.png)
...
![Chain detail view showing job sequence, input, and output](../../../assets/dashboard/chain-detail.png)

Shows a specific chain with its full job sequence and blocker relationships.

- **Jobs section**: All jobs in the chain, ordered by creation. Each job is a clickable card. Completed jobs show input and output. Running jobs show attempt info and the worker processing them. Failed attempts show the error.
...
![Job list view showing jobs across chains with status filters](../../../assets/dashboard/jobs-list.png)
...
![Job detail view showing status, schedule, and input data for a pending job](../../../assets/dashboard/job-detail.png)
...
- **Info** -- Status, attempt count, created/scheduled/completed timestamps, worker ID, and attempt expiry.

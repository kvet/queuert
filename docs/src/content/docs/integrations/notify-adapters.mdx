...
<Tabs>
  ...
  <TabItem label="In-process">
    **Built-in** — no extra package. Imported from `queuert`.
    ...
    Delivers job-arrival signals via in-memory subscriptions. Useful whenever publisher and
    subscriber run in the same process. Single-process only — workers in other processes won't

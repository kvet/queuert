  ...
  processors: {
    "orders.create": {
      attemptHandler: async ({ job, complete }) =>
        complete(async ({ finish }) => finish({ continueWith: { typeName: "orders.fulfill", input: { orderId: "123" } } })),
    },
    "orders.fulfill": {
      attemptHandler: async ({ job, complete }) => complete(async ({ finish }) => finish({ output: { fulfilled: true } })),
    },
  },
});
```
...
- [Utilities](/queuert/api/core/readme/) — API reference for `defineJobTypes`, `createJobTypes`, `createProcessors`

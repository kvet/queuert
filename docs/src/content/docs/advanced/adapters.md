...
- **Read-only methods leave it optional** (`getChains`, `getChainsByDeduplication`, `getJobs`, `getJobBlockers`, `getStartAttemptDelayMs`, `listChains`, `listJobs`, `listChainJobs`, `listBlockedJobs`). With a `txCtx` the read joins the caller's transaction and sees its uncommitted writes; without one the adapter acquires its own connection, executes, and releases.
- **`lock: "exclusive"` requires it.** `getChains`, `getChainsByDeduplication`, and `getJobs` do not mutate rows, but a write-intent lock only lasts as long as the transaction that took it, so the parameter type pairs `lock` with a mandatory `txCtx`.

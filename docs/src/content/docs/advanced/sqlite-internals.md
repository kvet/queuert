...
| `continued_to_job_id` | same as `id`                   | FK to the next job in the chain — non-null when the job has a successor (set in the same savepoint as the INSERT) |
| `input`               | `TEXT`                         | Job input as JSON string                                                                                          |
| `output`              | `TEXT`                         | Completion output as JSON string                                                                                  |
| `has_open_blockers`   | `INTEGER`                      | Denormalized 0/1 flag: 1 while ≥1 blocker chain is still open (gates acquisition)                                 |
...
- **No stored status**: status is derived at read time from structural columns (`completed_at`, `continued_to_job_id`, `leased_until`, `has_open_blockers`, `scheduled_at`); there is no `status` column or CHECK constraint
...
| ----------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `job_acquisition_idx`         | `(type_name, scheduled_at) WHERE has_open_blockers = 0 AND leased_until IS NULL AND completed_at IS NULL` | Job acquisition (ready/scheduled)                                        |
| `chain_index_idx`             | `UNIQUE (chain_id, chain_index)`                                                                          | Chain position uniqueness; `ON CONFLICT` race-decider for `continueWith` |
| `continued_to_job_id_idx`     | `UNIQUE (continued_to_job_id) WHERE continued_to_job_id IS NOT NULL`                                      | Successor uniqueness; `listChainJobs` cursor decode                      |
...
| `job_expired_lease_idx`       | `(type_name, leased_until) WHERE leased_until IS NOT NULL AND completed_at IS NULL`                       | Lease reaping                                                            |
| `job_chain_tail_idx`          | `(chain_id) WHERE continued_to_job_id IS NULL`                                                            | Chain frontier (tail) lookup                                             |
...
```sql
UPDATE job
SET leased_by = ?, leased_until = datetime('now', 'subsec', '+' || (? / 1000.0) || ' seconds'), attempt = attempt + 1
WHERE id = (
  SELECT id FROM job
...
- **`addJobBlockers`**: Separate INSERT for blockers, then UPDATE to set `has_open_blockers`
...
`listChains` joins each root row with the last job in the chain. The `status` filter (`open`/`closed`) applies to the joined last job and cannot use an index — only `typeName` and date range filters narrow the scan before the join. Without these filters, every root row is scanned and joined. On deployments with frequent writes, unfiltered scans over large tables can extend write queue wait times because the read lock is held longer.
